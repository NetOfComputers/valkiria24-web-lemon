...
function BirdViewAdmin() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      newSocket.emit('servicesStatus', {
        catchEventOn: 'servicesStatusReturn'
      });
      console.log('emitted servicesStatus');
    });
    ...
    newSocket.on('servicesStatusReturn', (dto) => {
      console.log('ServicesStatus', dto.stdout);

      if(dto.stdout.udpVideoPump || dto.stdout.udpAudioPump){
        alert('Other person is playing the video or audio. Wait til ends')
      }else {
        //Pretend server to wake up that services - Requires elevation -
      }
    });
